...
<html lang="en">
    ...
    <body>
		...
		<div class="panel panel-default sammacmedia text-center" style="margin:30px; padding:20px">
        ...
        <div class="panel-body" >
                        <table style="margin:30px 50px 30px 10px;"  class="table table-striped thead-dark table-bordered table-hover text-center" id="myTable">
                <thead>
                <tr>
                    <th>#</th>
                    <th>File Name</th>
                    ...
                    <?php
                                   $a=1;
                    $query=mysqli_query($mysqli,"select *from `files` ");
                     while($row=mysqli_fetch_array($query))
                        {
                          ...
                          <tr>
                            ...
                            <?php
                            if($row['status']=="open"){
                            echo "<p  class='btn btn-success'>New</p>";         
						}
						elseif($row['status']=="process"){
                            echo "<p  class='btn btn-info'>In Processing</p>";         
						}
						elseif($row['status']=="reopen"){
                            echo "<p  class='btn btn-success'>New</p>";         
						}
						elseif($row['status']=="reject"){
                            echo "<p  class='btn btn-warning'>Reject</p>";         
						}
                            else{
                            echo "<p  class='btn btn-danger'>Close</p>";
                            }     
                            ?>  
                            ...
                            <a href="#samstrover<?php echo $row['id']; ?>" data-toggle="modal" class="btn btn-warning"><span class="fa fa-pencil"></span> View</a> || <a href="all_employees.php?edited=1&idx=<?php echo $row['id']; ?>" data-toggle="modal" class="btn btn-danger"><span class="fa fa-times"></span> Remove</a>
                              ...
                              <td><a class="btn btn-warning" href="edit.php?id=<?php echo $row['id']; ?>"><span class="fa fa-pencil"></span> Modify</a></td>
                      ...
                      {
                          $id = $_GET['idx'];
                          if ($stmt = $mysqli->prepare("DELETE FROM files WHERE id = ? LIMIT 1"))
                          {
